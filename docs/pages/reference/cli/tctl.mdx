...
|`--latitude`|*no default*|Center latitude for geo-filtered search (decimal degrees). Requires --longitude and --radius.|
|`--limit`|`100`|Maximum number of events to return (0 for unlimited).|
|`--longitude`|*no default*|Center longitude for geo-filtered search (decimal degrees). Requires --latitude and --radius.|
...
|`--[no-]skill`|`false`|Print a Markdown skill describing how an LLM agent should use this command, then exit. All other flags are ignored.|
...
|`--radius`|*no default*|Radius in kilometers around the geo center. Requires --latitude and --longitude.|
