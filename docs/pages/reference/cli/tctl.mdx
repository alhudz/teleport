...
|`--exclude-source`|*no default* (any of (repeatable): `aws`, `github`, `okta`, `teleport`)|Exclude event_source values (repeatable).|
|`--exclude-status`|*no default* (any of (repeatable): `success`, `error`)|Exclude status values (repeatable).|
...
|`--format`|`yaml` (one of: `json`, `yaml`)|Output format. (Values: json, yaml)|
...
|`--query`|*no default*|Raw Lucene query. Mutually exclusive with structured filter flags. Example: --query 'identity_id:"alice@example.com" AND NOT status:"error"'|
...
|`--source`|*no default* (any of (repeatable): `aws`, `github`, `okta`, `teleport`)|Filter by event source (repeatable).|
|`--status`|*no default* (any of (repeatable): `success`, `error`)|Filter by event status (repeatable).|
