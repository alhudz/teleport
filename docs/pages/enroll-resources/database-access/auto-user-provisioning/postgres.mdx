...
use the `db_roles` role option. To create the role below, open a file called
`db-user-creator.yaml` with the following content:
...
Use `spec.allow.db_permissions` section in a user role to specify object
permissions given user should have. To create the role below, open a file called
`all-table-reader.yaml` with the following content:
