...
VNet is available on macOS, Windows, and Linux in Teleport Connect and tsh.
...
<TabItem label="Linux">
- A client machine running a Linux distro with systemd, systemd-resolved, polkit, and D-Bus.
- [Teleport Connect](teleport-connect.mdx), version 18.8.0 or higher.
</TabItem>
</Tabs>
...
<details>
<summary>System files required for VNet on Linux</summary>
VNet on Linux requires a systemd unit, a polkit policy, and a D-Bus service
file to be installed on the system. These files are bundled with the
[Teleport Connect](teleport-connect.mdx) package and deployed during installation.
If you use `tsh vnet` without Teleport Connect, you should install these files
manually.
See [`examples/systemd/vnet`](https://github.com/gravitational/teleport/tree/branch/v(=teleport.major_version=)/examples/systemd/vnet)
in the Teleport repository.
</details>

## Troubleshooting
{/* TODO(tangyatsu): add Linux specific troubleshooting */}
...
### Timeouts when trying to reach a Teleport cluster (macOS only)
...
<TabItem label="Linux">
```code
$ resolvectl query <Var name="tcp-app.teleport.example.com" />
...
<TabItem label="Linux">
To enable debug logs in VNet, first stop Teleport Connect. Then edit the VNet systemd service to
...
In the editor add the following, then save and exit:
...
Restart the service to pick up the new setting:

```code
$ sudo systemctl restart teleport-vnet.service
```

Next, start capturing VNet logs into a file:

```code
$ journalctl -fu teleport-vnet.service > vnet.log
```

In a separate terminal, start Teleport Connect with debug logs enabled:

```code
$ teleport-connect --connect-debug
```

Next, attempt to reproduce the issue with VNet. When done, return to the terminal running
`journalctl` and press `Ctrl-C` to stop capturing.

Once that's done, disable debug logs in VNet by reverting the service:

```code
$ sudo systemctl revert teleport-vnet.service
```

To gather logs from Teleport Connect, attach all files from
`~/.config/Teleport Connect/logs/` together with `vnet.log` produced in the earlier step.
</TabItem>
</Tabs>
