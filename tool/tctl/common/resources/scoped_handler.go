...
// Unlike [Handler], get and delete operations receive an explicit scope via
// [scopes.QualifiedName], making it impossible to accidentally omit a scope.
//
// Create and update retain the classic [services.UnknownResource] signature because
// the scope travels in the YAML body, not in the CLI args.
type ScopedHandler struct {
	...
	// subKind is the sub-kind segment from the first CLI arg (empty if not provided).
	...
	// createHandler powers "tctl create". Scope comes from the YAML body.
	createHandler func(ctx context.Context, client *authclient.Client, raw services.UnknownResource, opts CreateOpts) error
	// updateHandler powers "tctl edit". Scope comes from the YAML body.
...
// A nil qn lists all resources of this kind (optionally filtered by subKind).
// A non-nil qn returns only the resource matching that scope and name.
...
//
// Note: this check is client-side and inherently racy — the resource's scope could change
