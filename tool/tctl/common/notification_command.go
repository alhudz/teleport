...
func (n *NotificationCommand) Initialize(app *kingpin.Application, _ *tctlcfg.GlobalCLIFlags, _ *servicecfg.Config) {
	...
	n.create.Flag("format", "Output format.").Default(teleport.Text).EnumVar(&n.format, teleport.Text, teleport.JSON, teleport.YAML)
	...
	n.ls.Flag("user", "Set which user to list user-specific notifications for, or leave empty to list your own.").StringVar(&n.user)
	n.ls.Flag("format", "Output format.").Default(teleport.Text).EnumVar(&n.format, teleport.YAML, teleport.JSON, teleport.Text)
...
func (n *NotificationCommand) Create(ctx context.Context, client *authclient.Client) error {
	...
	if n.user != "" {
		...
		return n.outputCreatedNotification(created, fmt.Sprintf("Created notification %s for user %s\n", created.GetMetadata().GetName(), n.user))
	}
	...
	if len(n.roles) != 0 {
		...
		if n.requireAllRoles {
			return n.outputCreatedNotification(created, fmt.Sprintf("Created notification %s for users with all of the following roles: %s\n", created.GetMetadata().GetName(), n.roles))
		}

		return n.outputCreatedNotification(created, fmt.Sprintf("Created notification %s for users with one or more of the following roles: %s\n", created.GetMetadata().GetName(), n.roles))
	}
	...
	return n.outputCreatedNotification(created, fmt.Sprintf("Created notification %s for all users\n", created.GetMetadata().GetName()))
}

// outputCreatedNotification renders a freshly created notification (either a
// user notification or a global notification) in the configured output format.
// For text, the existing human-readable prose is preserved; for json/yaml the
// created object returned by the API is serialized.
func (n *NotificationCommand) outputCreatedNotification(created any, textMsg string) error {
	...
	case teleport.YAML:
		return trace.Wrap(utils.WriteYAML(n.stdout, created))
	default:
		return trace.BadParameter("unsupported format %q", n.format)
	}
}
