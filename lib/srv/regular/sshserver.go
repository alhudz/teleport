...
func (s *Server) dispatch(ctx context.Context, ch ssh.Channel, req *ssh.Request, serverContext *srv.ServerContext) error {
	...
	if s.proxyMode {
		...
		case sshutils.AgentForwardRequest:
			...
			err := s.handleAgentForwardProxy(ctx, req, serverContext)
			if err != nil {
...
func (s *Server) handleAgentForwardProxy(ctx context.Context, _ *ssh.Request, scx *srv.ServerContext) (err error) {
	event := scx.GetAgentForwardEvent()
	defer func() {
		if err != nil {
			event.Metadata.Code = events.AgentForwardFailureCode
	...
	return trace.Wrap(s.enableProxyAgentForward(scx))
}

// enableProxyAgentForward enables agent forwarding for the proxy without
// emitting an audit event. Used by handleProxyJump where Teleport
// initiates the forward.
func (s *Server) enableProxyAgentForward(scx *srv.ServerContext) error {
	if err := s.authHandlers.CheckAgentForward(scx); err != nil {
		return trace.Wrap(err)
	}
	scx.Parent().SetForwardAgent(true)
...
func (s *Server) handleProxyJump(ctx context.Context, ccx *sshutils.ConnectionContext, identityContext srv.IdentityContext, ch ssh.Channel, req sshutils.DirectTCPIPReq) {
	...
	if services.IsRecordAtProxy(recConfig.GetMode()) {
		err = s.enableProxyAgentForward(scx)
		if err != nil {
