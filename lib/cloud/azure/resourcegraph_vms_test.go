...
func TestQueryVirtualMachinesSanitize(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			name: "valid parameters: lowercase and deduplication",
			params: QueryLinuxVMsParams{
				SubscriptionID: uuid1,
				ResourceGroup:  "RG1",
				Locations:      []string{"WestUS2", "westeurope"},
...
func TestParseVirtualMachines(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			name: "all rows malformed returns error and nil slice",
			data: []any{
				"not a map",
				func() map[string]any {
					row := makeRow("bad-vm")
		...
		{
			name:     "single malformed row returns error and nil slice",
			data:     []any{123},
			errCheck: require.Error,
			verify: func(t *testing.T, got []*VirtualMachine) {
				require.Nil(t, got)
		...
		{
			name: "row missing required id field returns error and nil slice",
			data: []any{
				func() map[string]any {
					row := makeRow("bad-vm")
					delete(row, "id")
		...
		{
			name: "row missing required location field returns error and nil slice",
			data: []any{
				func() map[string]any {
					row := makeRow("bad-vm")
					delete(row, "location")
		...
		t.Run(tc.name, func(t *testing.T) {
			got, err := parseVirtualMachines(t.Context(), logger, tc.data)
			tc.errCheck(t, err)
			tc.verify(t, got)
		})
	}
