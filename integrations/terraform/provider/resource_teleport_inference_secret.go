...
func (r resourceTeleportInferenceSecret) Create(ctx context.Context, req tfsdk.CreateResourceRequest, resp *tfsdk.CreateResourceResponse) {
	...
	id := inferenceSecretResource.Metadata.Name

	_, err = r.p.Client.SummarizerClient().GetInferenceSecret(ctx, id)
	if !trace.IsNotFound(err) {
		if err == nil {
			existErr := fmt.Sprintf("InferenceSecret exists in Teleport. Either remove it (tctl rm inference_secret/%v)"+
				" or import it to the existing state (terraform import teleport_inference_secret.%v %v)", id, id, id)

			resp.Diagnostics.Append(diagFromErr("InferenceSecret exists in Teleport", trace.Errorf(existErr)))
			return
		}
	...
	_, err = r.p.Client.SummarizerClient().CreateInferenceSecret(ctx, inferenceSecretResource)
	if err != nil {
		resp.Diagnostics.Append(diagFromWrappedErr("Error creating InferenceSecret", trace.Wrap(err), "inference_secret"))
		return
	}
	...
	for {
		...
		if trace.IsNotFound(err) {
			...
			if tries >= r.p.RetryConfig.MaxTries {
				...
				resp.Diagnostics.AddError(diagMessage, "inference_secret")
				return
			}
			continue
		}
	...
	plan.Attrs["id"] = types.String{Value: inferenceSecret.Metadata.Name}
...
func (r resourceTeleportInferenceSecret) Update(ctx context.Context, req tfsdk.UpdateResourceRequest, resp *tfsdk.UpdateResourceResponse) {
	...
	inferenceSecretBefore, err := r.p.Client.SummarizerClient().GetInferenceSecret(ctx, name)
	if err != nil {
		resp.Diagnostics.Append(diagFromWrappedErr("Error reading InferenceSecret", err, "inference_secret"))
		return
	}

	_, err = r.p.Client.SummarizerClient().UpsertInferenceSecret(ctx, inferenceSecretResource)
	if err != nil {
		resp.Diagnostics.Append(diagFromWrappedErr("Error updating InferenceSecret", err, "inference_secret"))
		return
	}
		var inferenceSecretI *summarizerv1.InferenceSecret
	...
	for {
		...
		if inferenceSecretBefore.GetMetadata().Revision != inferenceSecretI.GetMetadata().Revision || false {
			break
		}
...
func (r resourceTeleportInferenceSecret) Delete(ctx context.Context, req tfsdk.DeleteResourceRequest, resp *tfsdk.DeleteResourceResponse) {
	...
	err := r.p.Client.SummarizerClient().DeleteInferenceSecret(ctx, id.Value)
	if err != nil {
		resp.Diagnostics.Append(diagFromWrappedErr("Error deleting InferenceSecret", trace.Wrap(err), "inference_secret"))
		return
	}
