...
Check enrollment status or troubleshoot enrollment failures. This path is read-only — no files are written. Uses `TCTL` and `CLOUD` from Steps 1–2.

**Run the nodes report:**

```bash
$TCTL discovery nodes --cloud=<CLOUD> --last=24h
```

Show the command to the user before running it. Present the output table directly. Status values: `Online`, `Installed (offline)`, `Failed (exit code=N)`.
...
**If any failures exist**, show the command and run it:

```bash
$TCTL get user_tasks --format=json
```

Filter to `spec.state == "OPEN"`. For each open task extract and present:
- `spec.issue_type` — the error category
- affected instance names from `spec.discover_azure_vm.instances` or `spec.discover_ec2.instances`

Present as a brief plain-English summary, not raw JSON.

For resolution steps, refer to the [Auto-Discovery troubleshooting guide](https://goteleport.com/docs/enroll-resources/auto-discovery/servers/troubleshooting/).
